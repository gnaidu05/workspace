...
// `--vercel` emits one CommonJS file for the serverless function: the built page
// plus the parts of the engine the server needs to validate a plan. The function
// then requires a single local file — no cross-package ES modules, no reliance
// on the bundler tracing files outside api/.
const VERCEL = process.argv.includes('--vercel');
const SERVER_MODULES = ['data/airports.js', 'data/cities.js', 'geography.js', 'engine.js'];
...
const build = async () => {
  ...
  if (VERCEL) {
    ...
    for (const file of SERVER_MODULES) {
      engineParts.push(flatten(await readFile(path.join(PUBLIC, file), 'utf8'), file));
    }
    const page = await readFile(path.join(ROOT, 'docs', 'campusroute.html'), 'utf8');
    const bundle = `'use strict';
// GENERATED by scripts/build-single-file.js --vercel — do not edit by hand.
// The planner page and the validation half of the engine, in one CommonJS file
// so the serverless function has no dependency outside api/.

const PLANNER_PAGE = ${JSON.stringify(page)};

${engineParts.join('\n\n')}

module.exports = { PLANNER_PAGE, validateState };
`;
    const bundleTarget = path.join(ROOT, '..', 'api', '_campusroute-bundle.js');
    await writeFile(bundleTarget, bundle, 'utf8');
    console.log(`Wrote api/_campusroute-bundle.js (${(bundle.length / 1024).toFixed(0)} KB)`);
    return;
  }
